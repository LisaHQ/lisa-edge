...
under `dataset/` back it up, restore it or initialize OTBR. Production should
keep `OTBR_AUTO_RESTORE_DATASET=1` and `OTBR_AUTO_CREATE_NETWORK=0`. The units in
`systemd/` schedule dataset backups when OTBR is selected.
