...
if [ -f "$EDGE_REPO/install/bootstrap/phases/30-directories.sh" ]; then
  BOOTSTRAP_PHASES="$EDGE_REPO/install/bootstrap/phases"
else
  BOOTSTRAP_PHASES="$EDGE_REPO/bootstrap/phases"
fi

bash "$BOOTSTRAP_PHASES/30-directories.sh"
bash "$BOOTSTRAP_PHASES/40-core-service-prep.sh"
bash "$EDGE_REPO/ops/deploy/deploy.sh"
